...
zero_errors = ['0'*i+'.' for i in range(1,100)]
...
class MainWindow(qtw.QWidget):
    ...
    def keypad(self):
        ...
        container.layout().addWidget(btn_dot, 6, 0, 1, 4)
    ...
    def func_result(self):
        ...
        if fin_string.count('/0') > 0:
            self.result_field.setText('Деление на ноль')
        elif fin_string.count('..') > 0:
            self.result_field.setText('Ошибка')
        elif fin_string.count(' ') == len(fin_string):
            self.result_field.setText('Ошибка')
        elif fin_string.count('0') == len(fin_string):
            self.result_field.setText('Ошибка')
        # elif '0'*(2*fin_string.count('00')) + '.' > 0:
        #     self.result_field.setText('Ошибка')
        else:
